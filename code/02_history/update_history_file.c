...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/05 11:57:43 by blucken           #+#    #+#             */
/*   Updated: 2025/05/05 11:58:24 by blucken          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 * @file update_history_file.c
 * @brief Handles shell command history management and persistence
 * @details This file contains functions for managing the shell's
 * command history,
 * including reading from and writing to a history file, and updating
 * the in-memory
 * history structure.
 */
 ...
 * @details Determines where new history entries should begin by
 * counting existing
 ...
 */
static void	process_history_line(t_shell *shell, char *line)
{
	char	*tmp;
...
void	process_history_buffer(t_shell *shell, char *buf, \
ssize_t bytes)
{
	char	line[BUF_SIZE];
	int		i;
	...
	while (i < bytes)
	{
		if (buf[i] == '\n')
		{
			line[j] = '\0';
			if (j > 0)
				process_history_line(shell, line);
			j = 0;
		}
